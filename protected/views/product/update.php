<?php
$this->pageTitle = Yii::app()->name . ' - แก้ไขข้อมูลสินค้า';
$this->breadcrumbs = array(
    'จัดการสินค้า' => array('admin'),
    'แก้ไขข้อมูลสินค้า ' . '(' . $model->product_name . ')'
);
?>
...
<div class="form">
    ...
    <div class="row">
        ...
        <?php echo $form->hiddenField($model, 'product_old_tag', array('value' => $model->product_tag)); ?>
        <div>หากแท็กมีหลายคำให้คั่นด้วยคอมม่า (,) เช่น สินค้าขายดี,สินค้าคุณภาพ,ราคาถูก</div>
    </div>
    <div class="row">
        <?php echo $form->labelEx($model, 'product_image'); ?>
        <?php
        $c_md_img = count($modelImage);
        $url = 'http://' . Yii::app()->request->getServerName() . '/' . $this->mainFolder;
        if (!empty($c_md_img)) {
            foreach ($modelImage as $k_md_img => $v_md_img) {
                $urlImg = $url . '/images/products/thumbs/' . $v_md_img['pdimg_name'];
                ?>
                <div style="float:left;" id="thumbnails" class="thumbId_<?php echo $v_md_img['pdimg_id']; ?>">

                    <div class="t-img"><a href="javascript:delImgEdit(<?php echo $v_md_img['pdimg_id']; ?>)"></a>
                        <img src=" <?php echo $urlImg; ?>" width="120" />
                        <div>
                            <div>
                                <input type="radio" id="m_<?php echo $v_md_img['pdimg_id']; ?>" <?php
        if ($v_md_img['pdimg_album'] == 1)
        ...
        ?>  value="<?php echo $v_md_img['pdimg_id']; ?>/m" name="Productimage[main_img]">ตั้งเป็นรูปหลัก</div>
                        </div>
                    </div>
                </div>
                <?php
...
<?php //echo $form->error($model, 'product_image');     ?>
        <div id="area-figure">
            <div id="thumb" style="float:left;"></div>
            <div id="upload_image" style="float:left;">                    
                <span class="ic-camera">
<?php //echo CHtml::activeFileField($model, 'product_image',array('id'=>'input_imageupload'));     ?>
        ...
        <script type="text/javascript">
            ...
            function addImage(src, fid) {
                ...
                document.getElementById("thumb").appendChild(newD);
                if (newImg.filters) {
                    try {
            ...
            function delImgEdit(delId){
                if ($('#m_' + delId).prop('checked')) {
                    alert('ไม่สามารถลบรูปนี้ได้เพราะถูกตั้งเป็นรูปหลัก');
                } else {
                    if(confirm('ยืนยันการลบข้อมูล')){
                        $.get( "<?php echo $url; ?>/productImage/delete/"+delId, function( data ) {
                            $('.thumbId_'+delId).fadeOut();
                        });
                    }
                }
            }
